...
df_table_date = df_art_table[df_art_table.nltk_sentiment == False]
df_table_date_set = set(df_table_date['publishedat'])

print(len(df_art_table))
total_articles = len(df_table_date)
loop_count = 0
print("Starting sentiment analysis")
for date in df_table_date_set:
    conn, cursor = get_conn_info()
    df_table_date_one_day = df_table_date[(df_table_date.publishedat == date) ] 
    
    for index, row in df_table_date_one_day.iterrows():
        if(loop_count%10 == 0):
